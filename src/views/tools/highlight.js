...
function createNewMeasurement (mouseEventData) {
  ...
  const existingToolData = cornerstoneTools.getToolState(mouseEventData.event.currentTarget, toolType);
...
function onImageRendered (e) {
  ...
  const context = cornerstoneTools.drawing.getNewContext(eventData.canvasContext.canvas);
  ...
  // const config = draghighlight.getConfiguration();
  const config = {
  ...
  cornerstoneTools.drawing.draw(context, (context) => {
    ...
    cornerstoneTools.drawing.fillOutsideRect(context, eventData.element, data.handles.start, data.handles.end, options);
    ...
    cornerstoneTools.drawing.drawRect(context, eventData.element, data.handles.start, data.handles.end, options);
    ...
    // Define variables for the area and mean/standard deviation
      ...
      // cornerstoneTools.drawLinkedTextBox(context, element, data.handles.textBox, text,
      //   data.handles, textBoxAnchorPoints, color, lineWidth, 0, true);
      cornerstoneTools.drawTextBox(context, text, data.handles.textBox.x, data.handles.textBox.y, color);
  });
...
const draghighlight = cornerstoneTools.mouseButtonRectangleTool({
...
const draghighlightTouch = cornerstoneTools.touchTool({
  createNewMeasurement,
  onImageRendered,
  pointNearTool,
  pointInsideRect,
