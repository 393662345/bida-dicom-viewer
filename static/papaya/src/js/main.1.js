...
/*** Contror Methods ***/
// file.name:文件名,file.data:文件内容
papaya.Container.openViewer = function (files, callback) {
    papayaContainers[0].viewer.resetViewer();
    papayaContainers[0].display.drawProgress(0.1, "Loading");
	papayaContainers[0].viewer.setDataChangeCallback(callback);
    papayaContainers[0].viewer.loadImage(files);
};

papaya.Container.openViewerAll = function (studyId, serieId) {
    papayaContainers[0].viewer.resetViewer();
    papayaContainers[0].viewer.showSurfaceView = true;
    papayaContainers[0].display.drawProgress(0.1, "Loading");
    papayaContainers[0].viewer.loadImage([], studyId, serieId);
};

papaya.Container.openSurfaceViewer = function () {
    if (papayaContainers[0].viewer.surfaceView == null) {
        papayaContainers[0].display.drawProgress(0.1, "Loading");
        papayaContainers[0].viewer.loadSurface("sample_image_surf");
    } else {
        papayaContainers[0].viewer.surfaceView.showSurfacePlanes = true;

        //this.viewer.calculateScreenSliceTransformsEx();
        //this.viewer.updateScreenSliceTransforms();
        //this.viewer.surfaceView.updateActivePlanes();

        if (papayaContainers[0].preferences.showSurfacePlanes === "Yes") {
            papayaContainers[0].preferences.updatePreference("showSurfacePlanes", "No");
        } else {
            papayaContainers[0].preferences.updatePreference("showSurfacePlanes", "Yes");
        }
        papayaContainers[0].viewer.rotateViewsNoSurface();
        papayaContainers[0].resizeViewerComponents(true);
        //this.viewer.drawViewer(false, false);
    }
};

papaya.Container.closeSurfaceViewer = function () {
    papayaContainers[0].viewer.surfaceView.showSurfacePlanes = false;
...
papaya.Container.changeWL = function (WW, WL) {
    // 修改坐标位置
    papayaContainers[0].viewer.currentScreenVolume.setScreenRange(WL - (WW/2), WL + (WW/2));
    papayaContainers[0].viewer.axialSliceDraw = true;
    papayaContainers[0].viewer.coronalSliceDraw = true;
    papayaContainers[0].viewer.sagittalSliceDraw = true;
    // var self = papayaContainers[0];
	
    papayaContainers[0].viewer.drawViewer(true);
    //papayaContainers[0].resizeViewerComponents(true);
};

papaya.Container.changeWLOld = function (WW, WL) {
    // 修改坐标位置
    //papayaContainers[0].viewer.currentScreenVolume.setScreenRange((WL - WW) / 2, (WL + WW) / 2);
    papayaContainers[0].viewer.axialSliceDraw = true;
    papayaContainers[0].viewer.coronalSliceDraw = true;
    ...
    $.ajax({
        url: MainUrl + 'api/Volume/PostMPROpt',
        data: JSON.stringify({ Id: ImageUID, Tag: 101, WL: WW, WW: WL}),
        cache: false,
        async: false,
        type: "POST",
        ...
        success: function (result) {
            ...
            var data = result;
            if (data.Image0) {
                self.viewer.volume.refashImgs(data);
            }
        }
     });
    papayaContainers[0].viewer.drawViewer(true);
...
papaya.Container.showRulers  = function (show) {
    // console.log('main', this)
    if(show)
        papayaContainers[0].preferences.updatePreference("showRuler", "Yes");
	else
        papayaContainers[0].preferences.updatePreference("showRuler", "No");
	// if(show)
...
papaya.Container.changeThicknessMM = function (ThicknessMM) {
    // 修改坐标位置
    var self = papayaContainers[0];
    jQuery.support.cors = true;
    $.ajax({
        url: MainUrl + 'api/Volume/PostMPROpt',
        data: JSON.stringify({ Id: ImageUID, Tag: 102, ThicknessMM: ThicknessMM }),
        cache: false,
        async: false,
        type: "POST",
        contentType: 'application/json',
...
papaya.Container.changeMRPMode = function (renderMode) {
    // 修改坐标位置
    var self = papayaContainers[0];
    jQuery.support.cors = true;
    $.ajax({
        url: MainUrl + 'api/Volume/PostMPROpt',
        data: JSON.stringify({ Id: ImageUID, Tag: 103, RenderMode: renderMode }),
        cache: false,
        async: false,
        type: "POST",
        contentType: 'application/json',
...
papaya.Container.changeRenderMode = function (vrPresetType) {
    // 修改3D渲染模式
    var self = papayaContainers[0];
    jQuery.support.cors = true;
    $.ajax({
        url: MainUrl + 'api/Volume/Post3DOpt',
        data: JSON.stringify({ Id: ImageUID, Tag: 601, VRPresetType: vrPresetType }),
        cache: false,
        async: false,
        type: "POST",
        contentType: 'application/json',
        success: function (result) {
            ...
            var data = result;
            if (data.Image) {
                //self.volume.refashImgs(data);
                var imageData = window.atob(data.Image);
                //var buf = papaya.utilities.StringUtils.stringToArrayBuffer(imageData);
                var charData = imageData.split('').map(function (x) { return x.charCodeAt(0); });
                // Turn number array into byte-array
                var binData = new Uint8Array(charData);
                // // unzip
                self.viewer.surfaces[0].imgs = pako.inflate(binData);
                self.viewer.surfaces[0].width = data.Width;
                self.viewer.surfaces[0].height = data.Height;
            }
        }
    });
    papayaContainers[0].resizeViewerComponents(true);
...
papaya.Container.changeCameraAngle = function (cameraAngle) {
    // 修改3D相机角度
    var self = papayaContainers[0];
    jQuery.support.cors = true;
    $.ajax({
        url: MainUrl + 'api/Volume/Post3DOpt',
        data: JSON.stringify({ Id: ImageUID, Tag: 602, CameraAngle: cameraAngle }),
        cache: false,
        async: false,
        type: "POST",
        contentType: 'application/json',
...
// 0: 正常
// 1：缩放
papaya.Container.changeMPRMouseType = function (mouseType) {
    // 修改操作方式
    // var self = papayaContainers[0];
    if (mouseType == 0)
        papayaContainers[0].preferences.scrollBehavior = 'Increment Slice';
    if (mouseType == 1)
        papayaContainers[0].preferences.scrollBehavior = 'Zoom';
    if (mouseType == 2) {
        //papayaContainers[0].preferences.scrollBehavior = 'Drop';
        isDragMode
        keyDownEvent
    }
};
...
// 0: 正常
// 1：拖动
papaya.Container.changeMPRDrawType = function (drawType) {
    // 修改操作方式
    // var self = papayaContainers[0];
    if (drawType == 0)
        papayaContainers[0].viewer.isShiftKeyDown = false;
    if (drawType == 1)
        papayaContainers[0].viewer.isShiftKeyDown = true;
    
};

//回到原点
papaya.Container.gotoCenter = function () {
    // 修改操作方式
    var self = papayaContainers[0];
    var center = new papaya.core.Coordinate(Math.floor(self.viewer.volume.header.imageDimensions.xDim / 2),
                    Math.floor(self.viewer.volume.header.imageDimensions.yDim / 2),
                    Math.floor(self.viewer.volume.header.imageDimensions.zDim / 2));
    self.viewer.gotoCoordinate(center);

};

//交换主窗体
papaya.Container.swap = function () {
    // 修改操作方式
    var self = papayaContainers[0];
    self.viewer.rotateViews();
