...
papaya.volume.Volume.prototype.readFiles = function (files, callback) {
    console.log('readfile', files, callback)
    this.files = files;
...
papaya.volume.Volume.prototype.readFilesEx = function (files, callback) {
    this.files = files;
    this.fileName = files[0].name;
    console.log('i go read file ex', this.fileName)

    this.onFinishedRead = callback;
    this.compressed = this.fileIsCompressed(this.fileName);
    this.fileLength = this.files[0].size;
    this.readNextFileEx(this, 0);
};

papaya.volume.Volume.prototype.readNextFile = function (vol, index) {
    console.log('i go read file')
    var blob;
...
papaya.volume.Volume.prototype.readNextFileEx = function (vol, index) {
    var blob;

    if (index < this.files.length) {
        blob = this.files[index].data;
        ...
        try {
            ...
            reader.onloadend = papaya.utilities.ObjectUtils.bind(vol, function (evt) {
                if (evt.target.readyState === FileReader.DONE) {
                    vol.rawData[index] = evt.target.result;
                    setTimeout(function () {vol.readNextFileEx(vol, index + 1); }, 0);
                }
            });
