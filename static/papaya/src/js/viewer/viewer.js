...
papaya.viewer.Viewer.prototype.onDataChangeEvent = function (data){
	if(data.type==1)
	    alert(data.val);
	//if(data.type==0)
	//    alert(data.progress);
...
papaya.viewer.Viewer.prototype.setDataChangeCallback = function (callback){
	this.container.display.onDataChange = callback;
	this.onDataChange = callback;
};

papaya.viewer.Viewer.prototype.loadImage = function (refs, forceUrl, forceEncode, forceBinary) {
	// 测试通知
	//this.container.display.onDataChange = this.onDataChangeEvent;
...
papaya.viewer.Viewer.prototype.loadBaseImage = function (refs, forceUrl, forceEncode, forceBinary) {
    ...
    if (forceBinary) {
		...
		this.volume.readFilesEx(refs, papaya.utilities.ObjectUtils.bind(this, this.initializeViewer));
    }else {
...
papaya.viewer.Viewer.prototype.initializeViewer = function () {
    ...
    if (this.volume.hasError()) {
        ...
        this.canvas.addEventListener("mousemove", this.listenerMouseMove, { passive: false });
        this.canvas.addEventListener("mousedown", this.listenerMouseDown, { passive: false });
        this.canvas.addEventListener("mouseout", this.listenerMouseOut, { passive: false });
        this.canvas.addEventListener("mouseleave", this.listenerMouseLeave, { passive: false });
        this.canvas.addEventListener("mouseup", this.listenerMouseUp, { passive: false });
        document.addEventListener("keydown", this.listenerKeyDown, true);
        document.addEventListener("keyup", this.listenerKeyUp, true);
        this.canvas.addEventListener("touchmove", this.listenerTouchMove, { passive: false });
        this.canvas.addEventListener("touchstart", this.listenerTouchStart, { passive: false });
        this.canvas.addEventListener("touchend", this.listenerTouchEnd, { passive: false });
        this.canvas.addEventListener("dblclick", this.listenerMouseDoubleClick, { passive: false });
        document.addEventListener("contextmenu", this.listenerContextMenu, { passive: false });
...
papaya.viewer.Viewer.prototype.addScroll = function () {
    // if (!this.container.nestedViewer) {
        window.addEventListener(papaya.utilities.PlatformUtils.getSupportedScrollEvent(), this.listenerScroll, { passive: false });
    // }
};
...
papaya.viewer.Viewer.prototype.drawScreenSlice = function (slice) {
    ...
    if (slice === this.surfaceView) {
        var viewerDim = this.canvas.height;
        var viewerWidth = this.canvas.width;
        var s = 0;


        if (this.container.hasSurface()) {
            if (viewerWidth > (viewerDim * 1.333))
                s = (viewerWidth - (viewerDim * 1.333)) / 4;
        } else {
            if (viewerWidth > (viewerDim * 1.5))
                s = (viewerWidth - (viewerDim * 1.5)) / 4;
        }
        this.context.fillStyle = this.surfaceView.getBackgroundColor();
        this.context.fillRect(slice.screenOffsetX - s, slice.screenOffsetY, slice.screenDim+(2*s), slice.screenDim);
...
papaya.viewer.Viewer.prototype.calculateScreenSliceTransforms = function () {
    ...
    if (this.container.orthogonalTall) {
        ...
        if (this.container.hasSurface()) {
            if (viewerWidth > (this.viewerDim * 1.333))
                s = (viewerWidth - (this.viewerDim * 1.333)) / 4;
            this.getTransformParameters(this.mainImage, this.viewerDim, false, 3);
            this.mainImage.screenTransform[0][2] += this.mainImage.screenOffsetX = s;
            this.mainImage.screenTransform[1][2] += this.mainImage.screenOffsetY = 0;

            this.getTransformParameters(this.lowerImageTop, this.viewerDim, true, 3);
            this.lowerImageTop.screenTransform[0][2] += this.lowerImageTop.screenOffsetX =
                (this.viewerDim + (papaya.viewer.Viewer.GAP)) + (s * 3);
            ...
            if (viewerWidth > (this.viewerDim * 1.5))
                s = (viewerWidth - (this.viewerDim * 1.5)) / 4;
            this.getTransformParameters(this.mainImage, this.viewerDim, false, 2);
            this.mainImage.screenTransform[0][2] += this.mainImage.screenOffsetX = s;
            this.mainImage.screenTransform[1][2] += this.mainImage.screenOffsetY = 0;
